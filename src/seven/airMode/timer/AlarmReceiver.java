...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (alarm == null) {
            LogHelper.d( TAG +"AlarmReceiver failed to parse the alarm from the intent");
            return;
        }
        ...
        LogHelper.d( TAG +"AlarmReceiver.onReceive() id " + alarm.id + " setFor "
                + format.format(new Date(alarm.time)));

        if (now > alarm.time + STALE_WINDOW * 1000) {
                LogHelper.d(TAG+"AlarmReceiver ignoring stale alarm");
            return;
        }
		...
		*/
        ...
        //取消看看是不是 Klaxon,结果是的，这是用来发声音的。但tm上面说它不是，而是震动。
       ...
       // context.startService(playAlarm);
        ...
        //TMD 不要弹出notification what't fuck,why so complex in one alarm..
       //NotificationManager nm = getNotificationManager(context);
        //nm.notify(alarm.id, n);
        
        //这里用来打开和关闭airmode,根据vibrate的值。 holllllllly shit about this.
        if(alarm.vibrate){
    ...
    private void updateNotification(Context context, Alarm alarm, int timeout) {
        ...
        if (alarm == null) {
            	LogHelper.d( TAG +"Cannot update notification for killer callback");
            return;
        }
